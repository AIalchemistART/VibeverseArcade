/**
 * SignEntity7.js - Ceiling sign entity for Circuit Sanctum Arcade
...
class SignEntity7 extends Entity {
    /**
     ...
     */
    constructor(x, y, width = 4.0, height = 4.0, signKey = 'sign7') {
        super(x, y, width, height);
        
        debug(`SignEntity7: Creating new ceiling sign at (${x}, ${y}) with key ${signKey}`);
        ...
        if (existingAsset) {
            debug(`SignEntity7: Found existing asset for ${signKey} in asset loader`);
            this.signImage = existingAsset;
        } else {
            debug(`SignEntity7: No existing asset found for ${signKey}, will load directly`);
        }
    ...
    directLoadSignImage() {
        if (this.signImage) {
            debug(`SignEntity7: Sign image already loaded, skipping direct load`);
            return;
        }
        
        debug(`SignEntity7: Directly loading sign image (attempt ${this.loadAttempts + 1}/${this.maxLoadAttempts})`);
        
        // Create a new image directly
        const img = new Image();
        
        img.onload = () => {
            debug(`SignEntity7: Successfully loaded sign image directly (${img.width}x${img.height})`);
            this.signImage = img;
            
            // Also store in asset loader for other components
            assetLoader.assets[this.signKey] = img;
            debug(`SignEntity7: Stored sign image in asset loader with key ${this.signKey}`);
        };
        
        img.onerror = (err) => {
            console.error(`SignEntity7: Failed to load sign image directly`, err);
            
            this.loadAttempts++;
            if (this.loadAttempts < this.maxLoadAttempts) {
                debug(`SignEntity7: Will try alternative path (attempt ${this.loadAttempts + 1})`);
                // Try again with a slightly different path
                setTimeout(() => this.tryAlternativePath(), 200);
            } else {
                debug(`SignEntity7: All ${this.maxLoadAttempts} attempts failed, creating fallback`);
                this.createFallbackImage();
            }
        };
        
        // Use exact path with proper path resolution
        const exactPath = 'assets/decor/Sign_7.png';
        const resolvedPath = getAssetPath(exactPath);
        debug(`SignEntity7: Setting image src to resolved path: ${resolvedPath}`);
     ...
     */
    tryAlternativePath() {
        debug(`SignEntity7: Trying alternative path for sign image`);
        
        const alternativePaths = [
            'assets/decor/Sign_7.png',
            '../assets/decor/Sign_7.png',
            './assets/decor/Sign_7.png',
            '/assets/decor/Sign_7.png',
            'Sign_7.png'
        ...
        debug(`SignEntity7: Trying alternative path: ${alternativePath}`);
        
        const img = new Image();
        
        img.onload = () => {
            debug(`SignEntity7: Successfully loaded sign image from alternative path: ${alternativePath}`);
            this.signImage = img;
        ...
        img.onerror = (err) => {
            console.error(`SignEntity7: Failed to load sign image from alternative path: ${alternativePath}`, err);
            
            this.loadAttempts++;
            if (this.loadAttempts < this.maxLoadAttempts) {
                debug(`SignEntity7: Will try another path (attempt ${this.loadAttempts + 1})`);
                // Try again with a slightly different path
     ...
     */
    createFallbackImage() {
        debug(`SignEntity7: Creating fallback sign image`);
        ...
        fallbackImg.onload = () => {
            debug(`SignEntity7: Fallback image created successfully`);
    ...
    drawFallbackSign(ctx, screenX, screenY, width, height, zOffset) {
        debug(`SignEntity7: Drawing fallback ceiling sign`);
    ...
    draw(ctx, screenX, screenY, width, height, zOffset) {
        debug(`SignEntity7: Drawing ceiling sign at (${this.x}, ${this.y}) -> screen (${screenX}, ${screenY})`);
        ...
        try {
            ...
            if (signImage) {
                debug(`SignEntity7: Image found, drawing actual ceiling sign`);
                ...
                debug(`SignEntity7: Ceiling sign image drawn successfully`);
            } else {
                debug(`SignEntity7: No image available, drawing fallback`);
            ...
            console.error(`SignEntity7: Error drawing ceiling sign:`, err);
            this.drawFallbackSign(ctx, screenX, screenY, width, height, zOffset);
        }
    }
}
