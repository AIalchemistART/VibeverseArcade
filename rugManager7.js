/**
 * RugManager7.js - Manager for seventh rug entities in Circuit Sanctum Arcade
 * Handles rug entity creation and placement for tertiary rugs
 */

import { RugEntity7 } from './rugEntity7.js';
import assetLoader from './assetLoader.js';
import { debug } from './utils.js';

export class RugManager7 {
    constructor(game) {
        this.game = game;
        this.rugs = [];
        
        debug('RugManager7: Initialized');
    }
 ...
 */
preloadAssets() {
    debug('RugManager7: Preloading rug assets');
    console.log('RugManager7: Preloading assets - starting');
    
    return new Promise((resolve) => {
        // Preload rug image
        const rugImage = new Image();
        rugImage.onload = () => {
            console.log('RugManager7: Rug image loaded successfully');
            // Important: Store in asset loader with the correct key
            assetLoader.assets['rug7'] = rugImage;
            resolve();
        };
            
        rugImage.onerror = (err) => {
            console.error('RugManager7: Failed to load rug image', err);
            ...
            canvas.height = 180;
                ...
                assetLoader.assets['rug7'] = fallbackImg;
                console.log('RugManager7: Created fallback image after load error');
            ...
            const assetPath = './assets/decor/Rug_7.png';
            console.log('RugManager7: Loading from path:', assetPath);
    ...
    addRugs(count = 1, positions = null) {
        debug(`RugManager7: Adding ${count} rugs to scene`);
        ...
        rugPositions.forEach(pos => {
            const rug = new RugEntity7(pos.x, pos.y, pos.z || -0.1);
            this.game.addEntity(rug);
            this.rugs.push(rug);
            debug(`RugManager7: Added rug at (${pos.x}, ${pos.y}, ${pos.z || -0.1})`);
        });
    }
    ...
    const defaultPositions = [
        { x: 36, y: 19.5, z: -0.1 }, // Different position for sixth rug
    ];
