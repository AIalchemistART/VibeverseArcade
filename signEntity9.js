/**
 * SignEntity9.js - Ceiling sign entity for Circuit Sanctum Arcade
...
class SignEntity9 extends Entity {
    /**
     ...
     */
    constructor(x, y, width = 4.0, height = 4.0, signKey = 'sign9') {
        super(x, y, width, height);
        
        debug(`SignEntity9: Creating new ceiling sign at (${x}, ${y}) with key ${signKey}`);
        
        // Sign specific properties
        this.signKey = signKey;         // Key for the sign asset
        this.zHeight = 35;             // Higher z-height for ceiling mounting
        ...
        if (existingAsset) {
            debug(`SignEntity9: Found existing asset for ${signKey} in asset loader`);
            this.signImage = existingAsset;
        } else {
            debug(`SignEntity9: No existing asset found for ${signKey}, will load directly`);
        }
    ...
    directLoadSignImage() {
        if (this.signImage) {
            debug(`SignEntity9: Sign image already loaded, skipping direct load`);
            return;
        }
        
        debug(`SignEntity9: Directly loading sign image (attempt ${this.loadAttempts + 1}/${this.maxLoadAttempts})`);
        
        // Create a new image directly
        const img = new Image();
        
        img.onload = () => {
            debug(`SignEntity9: Successfully loaded sign image directly (${img.width}x${img.height})`);
            this.signImage = img;
            
            // Also store in asset loader for other components
            assetLoader.assets[this.signKey] = img;
            debug(`SignEntity9: Stored sign image in asset loader with key ${this.signKey}`);
        };
        
        img.onerror = (err) => {
            console.error(`SignEntity9: Failed to load sign image directly`, err);
            
            this.loadAttempts++;
            if (this.loadAttempts < this.maxLoadAttempts) {
                debug(`SignEntity9: Will try alternative path (attempt ${this.loadAttempts + 1})`);
                // Try again with a slightly different path
                setTimeout(() => this.tryAlternativePath(), 200);
            } else {
                debug(`SignEntity9: All ${this.maxLoadAttempts} attempts failed, creating fallback`);
                this.createFallbackImage();
            }
        };
        
        // Use exact path with proper path resolution
        const exactPath = 'assets/decor/Sign_9.png';
        const resolvedPath = getAssetPath(exactPath);
        debug(`SignEntity9: Setting image src to resolved path: ${resolvedPath}`);
     ...
     */
    tryAlternativePath() {
        debug(`SignEntity9: Trying alternative path for sign image`);
        
        const alternativePaths = [
            'assets/decor/Sign_9.png',
            '../assets/decor/Sign_9.png',
            './assets/decor/Sign_9.png',
            '/assets/decor/Sign_9.png',
            'Sign_9.png'
        ...
        debug(`SignEntity9: Trying alternative path: ${alternativePath}`);
        
        const img = new Image();
        
        img.onload = () => {
            debug(`SignEntity9: Successfully loaded sign image from alternative path: ${alternativePath}`);
            this.signImage = img;
        ...
        img.onerror = (err) => {
            console.error(`SignEntity9: Failed to load sign image from alternative path: ${alternativePath}`, err);
            
            this.loadAttempts++;
            if (this.loadAttempts < this.maxLoadAttempts) {
                debug(`SignEntity9: Will try another path (attempt ${this.loadAttempts + 1})`);
                // Try again with a slightly different path
     ...
     */
    createFallbackImage() {
        debug(`SignEntity9: Creating fallback sign image`);
        ...
        fallbackImg.onload = () => {
            debug(`SignEntity9: Fallback image created successfully`);
    ...
    drawFallbackSign(ctx, screenX, screenY, width, height, zOffset) {
        debug(`SignEntity9: Drawing fallback ceiling sign`);
    ...
    draw(ctx, screenX, screenY, width, height, zOffset) {
        debug(`SignEntity9: Drawing ceiling sign at (${this.x}, ${this.y}) -> screen (${screenX}, ${screenY})`);
        ...
        try {
            ...
            if (signImage) {
                debug(`SignEntity9: Image found, drawing actual ceiling sign`);
                
                // Draw sign image
                const signWidth = width * 1.4;
                const signHeight = height * 2.5;
                ...
                debug(`SignEntity9: Ceiling sign image drawn successfully`);
            } else {
                debug(`SignEntity9: No image available, drawing fallback`);
            ...
            console.error(`SignEntity9: Error drawing ceiling sign:`, err);
            this.drawFallbackSign(ctx, screenX, screenY, width, height, zOffset);
        }
    }
}
